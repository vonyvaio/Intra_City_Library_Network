...
<title>Buy Order Details</title>

</head>

<body>

${library_buy_order_details_message}
...
<%
	...
	try {
		...
		Query = "select * from buy_order_details where library_id = " + Integer.parseInt((String) session.getAttribute ("library_id")) + ";";
		st = con.prepareStatement(Query);
		rs = st.executeQuery();
		
		while (rs.next()) {
		
			buy_order_id = rs.getString("buy_order_id");
			book_id = rs.getString("book_id");
			library_id = rs.getString("library_id");
		
			customer_id = rs.getString("customer_id");
			category_string = rs.getString("category_string");
		
			buy_date = rs.getString("buy_date");
			quantity_purchased = rs.getString("book_quantity");
			...
			customer_address = rs1.getString("student_address");
				...
				Query = "select * from member_details where member_id = " + Integer.parseInt(customer_id) +";";
				st = con.prepareStatement(Query);
				rs1 = st.executeQuery();
				
				rs1.next();
				
				customer_name = rs1.getString("member_first_name");
				customer_name = customer_name + " " + rs1.getString("member_last_name");
				
				customer_email = rs1.getString("member_email");
				customer_phone = rs1.getString("member_contact_no");
				customer_address = rs1.getString("member_address");	
			}
